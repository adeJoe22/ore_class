...
const passwordUtils_1 = require("../../utils/auth/passwordUtils");
...
const registration = (input) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const checkUserExist = yield User_1.default.findOne({
            email: input.email,
        });
        if (checkUserExist !== null)
            throw new errors_1.Authorization_Error("User email unavailable");
        if (input.terms_condition !== true)
            throw new errors_1.Authorization_Error("You cannot continue until you agree with our terms and conditions");
        // Handle roles
        // let roles = ["reader"]; // Default role
        // if (input.roles && input.roles.length > 0) {
        //   roles = input.roles;
        // }
        const salt = yield (0, passwordUtils_1.GenerateSalt)();
        const hashedPassword = yield (0, passwordUtils_1.GenerateHash)(input.password, salt);
        const newUser = new User_1.default(Object.assign(Object.assign({}, input), { password: hashedPassword, salt, terms_condition: input.terms_condition }));
        // Generate OTP
        const otp = (0, auth_token_service_1.generateOTP)();
        const otpSalt = yield (0, passwordUtils_1.GenerateSalt)();
        const hashedOTP = yield (0, passwordUtils_1.GenerateHash)(otp, otpSalt);
        ...
        throw new errors_1.INTERNAL_SERVER_ERROR(error.message);
    }
});
...
const login = (email, password) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Generate tokens
        const accessToken = (0, auth_token_service_1.generateToken)({
            userId: user._id,
            exp: auth_token_service_1.accessExpiration,
            iat: Math.floor(Date.now() / 1000),
            email: user.email,
        });
        const refreshToken = (0, auth_token_service_1.generateToken)({
            userId: user._id,
            exp: auth_token_service_1.refreshExpiration,
            iat: Math.floor(Date.now() / 1000),
            email: user.email,
        });
        // Store refresh token
...
//   }
// };
const logout = (userId, refreshToken) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = yield User_1.default.findById(userId);
        if (!user) {
            throw new errors_1.Authorization_Error("User not found");
        }
